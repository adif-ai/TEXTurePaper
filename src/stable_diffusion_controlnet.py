    ...
    def img2img_step(self, text_embeddings, inputs, depth_mask, guidance_scale=100, strength=0.5,
        ...
        def sample(latents, depth_mask, strength, num_inference_steps, update_mask=None, check_mask=None,
            ...
            if latents is None:
                ...
                latents = torch.randn(
                    (
                        text_embeddings.shape[0] // 2, self.unet.in_channels, latent_w, latent_h),
                    device=self.device)
        ...
        # depth_mask = F.interpolate(depth_mask, size=(64, 64), mode='bicubic',
        #                            align_corners=False)
        depth_mask = F.interpolate(depth_mask, size=(512, 512), mode='bicubic', align_corners=False).expand(1, 3, 512, 512)
        masked_latents = None
        if inputs is None:
            latents = None
        elif latent_mode:
